...
#Remove Machine from Hybrid Azure-AD on Shutdown

$taskPath = "\Microsoft\Windows\Workplace Join\"
$taskName = "AAD-Leave-On-Shutdown"

$AADLeaveTask = $taskPath + $taskName

$AADLeaveTaskCheck = schtasks /Query /TN $AADLeaveTask /XML 2>$null
if (-not $AADLeaveTaskCheck) {
    # ============================================================
    # Build XML for shutdown-triggered task
    # ============================================================
...
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Description>Leave Azure AD on shutdown</Description>
  </RegistrationInfo>
  <Triggers>
    <EventTrigger>
...
<QueryList>
  <Query Id="0" Path="System">
    <Select Path="System">*[System[(EventID=1074 or EventID=1076 or EventID=6006 or EventID=6008)]]</Select>
  </Query>
</QueryList>
        ]]>
      </Subscription>
  ...
  <Actions Context="Author">
    <Exec>
      <Command>C:\Windows\System32\dsregcmd.exe</Command>
      <Arguments>/leave</Arguments>
    </Exec>
  </Actions>
</Task>
"@
    ...
    Write-Host "AAD Leave on Shutdown task created successfully." -ForegroundColor Green
}
else {
    Write-Host "AAD Leave on Shutdown task already exists." -ForegroundColor Green
}
